...
const particleCount = isMobile ? 800 : isTablet ? 1200 : 2000;
const maxClouds = isMobile ? 3 : isTablet ? 5 : 8;
const renderQuality = isMobile ? 1 : Math.min(window.devicePixelRatio, 2);
...
// Don't set scene background - let CSS handle it
// scene.background = gradientTexture;

// Create gradient texture for universe background
...
// Create multiple star fields with different sizes and colors
const starFields = [];
...
const starTexture = new THREE.CanvasTexture(createCircularTexture());
...
const createStarField = (count, size, color, range) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    starColor.multiplyScalar(0.5 + Math.random() * 0.5);
